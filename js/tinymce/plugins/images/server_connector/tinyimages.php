...
class tinyimages {
	...
	private function mainField() {
		...
		if ($handle = opendir ( DIR.$this->folder )) {
			while (false !== ($file = readdir ( $handle ))) {
				if (is_file(DIR.$this->folder.'/'.$file)) {
					...
					$ret[] = array(
						'src'	=> $this->folder.'/.thumbs/100x100_'.$file,
						'attr'	=> $attr,
		...
		if(count($ret) > 0) {
			...
			foreach ($ret as $val) {
				...
				$return .= '<div class="item" ondblclick="addImage(this,\''.$val['path'].'\','.$width_for_insert.');" onclick="activateItem(this,\''.$val['path'].'\');"><img src="'.$val['src'].'" width="100" height="100" alt=" " /><div class="labels">'.$val['width'].'x'.$val['height'].'</div><div class="labels">'.$val['size'].'</div></div>';
			}
	...
	function ajaxMakeFolder($input) {
		...
		if($input['lng']=='ru') {
			if(trim($input['name']) == '') return array('error' => '  ');
		} else {
